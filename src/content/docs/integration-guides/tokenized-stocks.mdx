...
sources:
- base/docs:docs/build-on-base/integrate-defi/list-tokenized-stocks.mdx
...
- Or skip the onchain reads: the public [Tokenized Stocks API](/api-reference/tokenized-stocks-api/) returns each token's address, symbol, name, decimals, icon URL, and ISIN in one request, with no key needed.
...
The [Tokenized Stocks API](/api-reference/tokenized-stocks-api/) exposes the same feed value as `nav_price`, next to `nav_price_updated_at` and the current `multiplier`. It inherits every limit above: it is a reference value, not a tradeable price, and `nav_price_updated_at` needs the same staleness bound as `updatedAt`.
...
The tables here are a snapshot. For a live copy, the [Tokenized Stocks API](/api-reference/tokenized-stocks-api/) serves the token contracts from its list endpoint and the protocol contracts from `/chains`.
...
<Aside type="caution">
Three tickers — COINc, CRCLc, and INTCc — sat in the tables above until recently and never launched. Their B20 addresses are registered, so `name` and `symbol` still answer normally, and their Chainlink feeds are still deployed and still publishing a price. Neither of those checks will reveal that the token is absent. Supply is what reveals it: each reads zero, because none was ever minted. If you are carrying one of these addresses from an earlier version of this page, read `totalSupply` before you treat the token as tradeable.
</Aside>

## Related pages

- [Tokenized Stocks API](/api-reference/tokenized-stocks-api/) — token records, supplies, multipliers, and NAV values over HTTP
