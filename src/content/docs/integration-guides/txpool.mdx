...
description: Working with the Base transaction pool.
---

The Base transaction pool (txpool) manages pending transactions before they are included in blocks. The `base/base` repository provides RPC extensions and tracing instrumentation for inspecting and monitoring pool state.
