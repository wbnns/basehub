...
1. **Ingress** -- transactions arrive via `eth_sendRawTransaction` through the proxy layer in front of the node. This used to be the `ingress-rpc` service, which was retired on 2026-09-21; the stage is the same, and nothing about the pool behaviour below depends on which component fronts it.
