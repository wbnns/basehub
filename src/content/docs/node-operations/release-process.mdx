...
**Build RC** is what actually produces the artifacts, delegating to `Build Release` for multi-arch Docker images and native binaries. Images go out under the RC tag alone; `latest` stays where it is until a final release. Because each candidate builds inside its own run, a fresh merge no longer cancels or queues behind the artifacts of the candidate before it — runner capacity permitting, the two build side by side.
...
- Compiles the `base` image a single time under `PROFILE=maxperf`, and hangs four tags off that one image on `ghcr.io/base/node`: `vX.Y.Z`, `X.Y`, `X`, and `latest`.
