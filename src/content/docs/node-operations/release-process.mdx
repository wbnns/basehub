...
- Publishes those images under the RC tag alone, leaving `latest` untouched.
...
- Runs a single `base` image build under `PROFILE=maxperf`, then tags that one image as `vX.Y.Z`, `X.Y`, `X`, and `latest` on `ghcr.io/base/node`.
- Opens a draft GitHub release, generates the changelog for it, and attaches the binaries.
...
| Build RC | **Create RC** | Automatic on push to `releases/v*` | RC tag + Docker images + binaries |
