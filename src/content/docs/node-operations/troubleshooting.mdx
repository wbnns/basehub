...
This guide covers the most common issues operators hit when running a Base node from the official [Docker setup](https://github.com/base/base), with steps to diagnose and resolve each one.
...
- **Issue:** `Error: nonce has already been used` when sending transactions.
  ...
  - **Tip:** [`basectl`](/node-operations/basectl/), the Base node operator CLI, surfaces richer node data and diagnostic checks than the raw RPC output.
...
- **Issue:** Low peer count, or the node cannot connect to any peers at all.
  - **Check:** Ingress ports `30303` (TCP/UDP) and `9222` (TCP/UDP for Reth discv5) are reachable from the public internet through any host firewall and upstream network filtering.
  - **Check:** Outbound traffic to ports `30301` (TCP/UDP) and `9200` (UDP) is permitted. These are the Base bootnode endpoints — if egress to them is blocked, the node has no entry point into peer discovery and will report zero peers even when ingress rules look correct.
...
- **GitHub:** Search the [base/base issue tracker](https://github.com/base/base/issues). File a fresh issue when the behaviour looks like a defect.
