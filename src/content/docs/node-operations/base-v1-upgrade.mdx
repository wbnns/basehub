...
<Aside type="note">
Both binaries now ship from [base/base](https://github.com/base/base/releases), which carries the root `docker-compose.yml` and the `.env.mainnet` / `.env.sepolia` templates. Most configuration arrives preset and environment variables override it. See those `.env` files for the complete option set.
...
If you already run OP Reth through the compose setup, updating to the latest release switches you to `base-reth-node` automatically. Your existing `./reth-data` directory is fully compatible — no resync or snapshot restore required.
...
2. Pull the latest [base/base](https://github.com/base/base), which now publishes the operator node:
...
The `.env.mainnet` and `.env.sepolia` templates in [base/base](https://github.com/base/base) already populate most variables. If you build from [base/base](https://github.com/base/base), translate `op-node` variables with the table below. Nearly all of them are optional. `base-consensus node --help` prints the complete set.
