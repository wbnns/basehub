...
<Aside type="note">
Both clients now ship from [base/base](https://github.com/base/base/releases). That repository holds the root `docker-compose.yml` along with the `.env.mainnet` and `.env.sepolia` templates operators used to take from `base/node`. Most settings arrive preset, and environment variables override them — see those `.env` files for every adjustable option.

`docker compose up` pulls the published `ghcr.io/base/node` image. Add `--build` to compile the tree you have checked out, or set `NODE_TAG=vX.Y.Z` to pin one released image and skip the compile. [Deployment](/node-operations/deployment/) covers both paths.
</Aside>
...
Operators already on OP Reth through the compose setup only need to update — the latest version switches the binary to `base-reth-node` automatically, and your current `./reth-data` directory carries over with no re-sync or snapshot restore required.
...
2. Move to [base/base](https://github.com/base/base), which now publishes the operator node, and update it:

   ```bash
   # Coming from a base/node clone? Clone base/base next to it and bring your env file across:
   git clone https://github.com/base/base.git
   cp ../node/.env.mainnet ./base/.env.mainnet   # or .env.sepolia

   # Already tracking base/base:
...
2. Clone [base/base](https://github.com/base/base), or update an existing clone:

   ```bash
   git clone https://github.com/base/base.git
   # or, where it is already cloned:
   git pull origin main
   ```
...
The `.env.mainnet` and `.env.sepolia` templates in [base/base](https://github.com/base/base) already populate most of these. If you build from [base/base](https://github.com/base/base), translate your `op-node` variables with the table below. Nearly all of them are optional. `base-consensus node --help` prints the complete set.
