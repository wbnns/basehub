...
Note the accessor: the trait declares its own `fork_condition`, so the trait method calls that rather than `RollupConfig`'s inherent `upgrade_activation`. Step 4 above still uses `upgrade_activation`, which remains in place on `RollupConfig`.
