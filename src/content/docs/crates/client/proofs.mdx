---
title: "client/proofs"
description: "Proofs extension for the Base node — fault proof data via Execution Extensions."
---

import { LinkCard } from '@astrojs/starlight/components';

## Overview

The `client/proofs` sub-crate (`base-proofs-extension`) provides the proofs extension for the Base node. It uses Reth's Execution Extensions (ExEx) framework to generate and serve fault proof data.

## Source

<LinkCard title="View source on GitHub" href="https://github.com/base/base/tree/main/crates/client/proofs" />
