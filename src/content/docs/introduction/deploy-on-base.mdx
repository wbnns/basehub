...
This lands `base-forge`, `base-cast`, `base-anvil`, and the `base-chisel` REPL next to whatever Foundry you already have. To target a specific Base release, pin it with `base-foundryup --install <version>`; releases are named for the Base chain release they support. Nothing is overwritten, and the two toolchains coexist.
