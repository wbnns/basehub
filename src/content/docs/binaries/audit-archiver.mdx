...
`audit-archiver` archives audit log messages to S3-compatible storage for long-term retention, debugging, and compliance. Its Kafka producer used to be `ingress-rpc`, which was removed from `base/base` on 2026-09-21; the archiver itself stayed, and it still ingests transaction events over HTTP from Vector.
