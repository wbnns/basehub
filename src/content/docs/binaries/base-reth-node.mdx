...
`just build node` is a debug build — it wraps the plain `cargo build --bin base-reth-node` above. Pass `--release` when you want the optimized binary. The workspace no longer carries a `maxperf` profile, so `release` is the production profile.
