...
`just build node` is a debug build — it wraps the plain `cargo build --bin base-reth-node` above. Pass `--release` when you want the optimized binary. That covers local use. The published binary archives are compiled with `maxperf` instead, a heavier variant of `release` that Base applies only to release artifacts.
