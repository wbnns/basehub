---
title: Prerequisites
description: Required tools — Rust, Just, Foundry, Docker.
---

Before building or developing on `base/base`, install the following tools. The instructions below target Linux (Debian/Ubuntu). macOS users can substitute `brew install` where applicable; the Rust and Foundry installers are cross-platform.
