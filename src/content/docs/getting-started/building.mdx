...
`release` is the profile you want for day-to-day work, and it is what `docker-bake.hcl` falls back to when nothing sets `PROFILE`. The workspace also keeps `maxperf`, which inherits `release` and then switches to fat LTO with a single codegen unit. Base reserves it for the artifacts it publishes, so reaching for it locally mostly buys you a much longer compile. Every profile is listed under [build profiles](/node-operations/configuration/#build-profiles).
