...
Two names are the exception, and they are already marked throughout this page. `burnBlocked` and `BURN_BLOCKED_ROLE` are deprecated. They still resolve, because nothing is removed, but seizure flows written today are expected to use [`seizeWithMemo`](#ib20) instead.

### Release history

The standard's first release rode in with the [Beryl upgrade](/specifications/beryl-overview/), and it carried the whole surface documented here — the roles model, the policy registry and its six per-token scopes, memos, granular pausing, supply caps, `permit`, the factory, and both the Asset and Stablecoin variants.
