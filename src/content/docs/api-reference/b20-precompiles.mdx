...
sources:
- base/docs:docs/base-chain/specs/reference/b20/interfaces/IB20.mdx
- base/docs:docs/base-chain/specs/reference/b20/interfaces/IB20Asset.mdx
- base/docs:docs/base-chain/specs/reference/b20/interfaces/IB20Stablecoin.mdx
- base/docs:docs/base-chain/specs/reference/b20/interfaces/IB20Factory.mdx
- base/docs:docs/base-chain/specs/reference/b20/interfaces/IPolicyRegistry.mdx
- base/docs:docs/base-chain/specs/reference/b20/interfaces/IActivationRegistry.mdx
- base/docs:docs/base-chain/specs/reference/b20/constants-and-addresses.mdx
- base/docs:docs/base-chain/specs/reference/b20/errors-and-events.mdx
- base/docs:docs/base-chain/specs/reference/b20/changelog.mdx
- base/docs:docs/base-chain/specs/reference/b20/changelog/02-cobalt-b20-seize.mdx
- base/docs:docs/base-chain/specs/reference/b20/changelog/02-cobalt-b20asset-multiplier.mdx
- base/docs:docs/base-chain/specs/reference/b20/changelog/02-cobalt-policyregistry-composite-policy.mdx
---
...
| `TRANSFER_EXECUTOR_POLICY` | `0x724e9c53` | `msg.sender` on `transferFrom`, but only where it differs from `from`. From Denim, `msg.sender` on every transfer path, `transfer` included ([details](/integration-guides/b20-issuer-operations/#limit-who-may-initiate-transfers)). |
...
Every selector on this page is final, but not all of them are dialable today. [Beryl](/specifications/beryl-overview/) shipped the standard and is live on Mainnet and Sepolia. A second group arrives with [Cobalt](/specifications/cobalt-overview/), now booked for Sepolia on September 23, 2026 and Mainnet on September 30, 2026 — call any of it before your target network reaches its date and there is nothing at the address to answer.
...
Three groups are specified and documented above, but undialable until Cobalt activates.
...
Three changes are already in the node source, gated behind [Denim](/specifications/denim-overview/) rather than Cobalt. Denim has a target month but no activation timestamp yet.
...
- On `transferFrom` the recipient is validated before the allowance is touched, so a call that trips this rule leaves the allowance unspent rather than consuming it on a revert.
- For seizure, this closes a disguised-burn path: because seize leaves `totalSupply` alone, seizing into the token address used to remove a balance from circulation without recording it as a burn.

**The executor policy covers every transfer.** Asset and Stablecoin both move to logic version 3 at Denim. Before it, `TRANSFER_EXECUTOR_POLICY` runs only on `transferFrom` where the caller is not `from`. From Denim it runs first on `transfer`, `transferFrom`, and the memo variants, so a holder who is not on a restrictive executor allowlist can no longer move their own balance. When the caller is `from` and both the executor and sender scopes hold the same policy ID, the sender check is skipped as redundant. Storage is append-only, so existing tokens switch in place with no migration. Upstream's issuer guide documents this behaviour without the Denim gate. See [B20 Issuer Operations](/integration-guides/b20-issuer-operations/#limit-who-may-initiate-transfers).
...
<Aside type="caution">
The same stored ID means two different things either side of Denim. Before the fork, bit 63 puts the type byte above `INTERSECT`, so the registry reads the ID as malformed: `isAuthorized` returns `false` for every account and `policyExists` returns `false`. After the fork, that ID authorizes everyone the base policy rejects. A slot bound to an inverted ID ahead of activation goes from deny-all to the complement at the fork block, with no transaction marking the change. And `invertedPolicyId` itself reverts with an unknown-selector error until then.
</Aside>

## What changes for integrators at Cobalt

Nothing you call today stops working, so none of this is a forced migration. The timing has tightened, though: Sepolia activates on September 23, 2026 and Mainnet a week later, so each item below is now a path worth moving to on a schedule rather than eventually.

### Seizure replaces `burnBlocked`

`burnBlocked` and `BURN_BLOCKED_ROLE` are deprecated but untouched — same selector, same events, same behaviour, still callable. `seizeWithMemo(from, to, amount, memo)` becomes the canonical route for administrative balance removal.

The two are not interchangeable, and swapping the selector alone will not reproduce the old outcome:

- **Seize is a transfer, not a burn.** It moves `amount` from `from` to `to` and leaves `totalSupply` alone. To get the old result, seize into a treasury address you control and then call `burn`. Do not seize into the token's own address: that reverts `InvalidReceiver` from Denim onward, and before Denim it strands the balance instead of burning it.
- **They read different policies.** `seizeWithMemo` consults `SEIZE_EXEMPT_POLICY`; `burnBlocked` consults `TRANSFER_SENDER_POLICY`. A token can define a seizable set that has nothing to do with its transfer-blocked set. Under both, "eligible" means *not* authorized by the relevant policy.
- **They sit in different role and pause domains.** `SEIZE_ROLE` and `BURN_BLOCKED_ROLE` are separate grants, and `SEIZE` (ordinal 3) and `BURN` (ordinal 2) are separate pause bits. Pausing one leaves the other running.

<Aside type="caution">
Seize is off until an issuer switches it on. An unset policy slot reads as always-allow, so with `SEIZE_EXEMPT_POLICY` unset no account qualifies as seizable and every `seizeWithMemo` call reverts `AccountNotSeizable`. A token whose issuer never configures that slot has no seize capability at all. Leaving `SEIZE_RECEIVER_POLICY` unset has the opposite effect — any destination is permitted.
</Aside>
...
`updateUIMultiplier(newMultiplier, effectiveAt)` is the path meant for corporate actions such as splits and reinvested dividends. Only one pending update exists at a time.

1. **Schedule** with `updateUIMultiplier`. It needs `OPERATOR_ROLE`, and `effectiveAt` must be strictly in the future.
2. **Observe** while it waits: `newUIMultiplier()` gives the target, `effectiveAt()` gives the flip time, and `uiMultiplier()` / `multiplier()` still report the value in force.
3. **Let it mature.** Once `block.timestamp >= effectiveAt` the reads flip on their own. No event marks the moment.
4. **Or drop it** with `cancelUIMultiplierUpdate()`, which emits `UIMultiplierUpdateCancelled`.
...
<Aside type="note">
Indexers only need to follow `UIMultiplierUpdated`. The instant setter emits it too, alongside the deprecated `MultiplierUpdated`, and emits `UIMultiplierUpdateCancelled` first if it displaced a pending update. Every multiplier change reaches that one stream, so nothing is missed. The legacy topic stays available for consumers that have not moved.
</Aside>

### Composite policies join existing ones

A composite authorizes by combining 2 to 4 existing simple policies under `UNION` (OR) or `INTERSECT` (AND). Build one with `createCompositePolicy(admin, policyType, childPolicyIds)`; change it with `updateComposite(policyId, childPolicyIds)`, which replaces the child set **in full** — there is no partial update and no way to clear the list.
...
- **A renounced child keeps working.** `renounceAdmin` on a child policy only clears its admin and freezes its membership; the policy still exists and still answers `isAuthorized`, so the parent composite evaluates it exactly as before.
- **`compositePolicyChildIds` never reverts.** Handed a simple policy or an ID that was never created, it returns an empty array rather than an error, so an empty result does not distinguish "not a composite" from "does not exist".

<Aside type="caution">
Check `policyExists(policyId)` before you write any ID into a B20 policy slot, and treat an `INTERSECT` ID as the case that punishes skipping it. Authorization reads the stored child list rather than a creation record, so an ID that was never created simply resolves to an empty list. Under `UNION` an empty list denies everyone, which fails closed and shows up immediately. Under `INTERSECT` the same empty list means no child ever declines, so the gate returns `true` for every account — a slot you believed was restrictive behaves like `ALWAYS_ALLOW`, and nothing reverts to tell you. A typo in a policy ID is enough to reach it.
</Aside>
...
Everything on this page is fixed. B20 method and event signatures sit on the chain's consensus surface, so a selector that ships never changes meaning and never gets withdrawn. The standard only grows by addition: a later release can introduce new calls, but it will not rewrite the behaviour behind one you already integrated against.
...
Two names are the exception, and they are already marked throughout this page. `burnBlocked` and `BURN_BLOCKED_ROLE` are deprecated. They still resolve, because nothing is removed, but seizure flows are expected to use [`seizeWithMemo`](#ib20) once Cobalt is live.
