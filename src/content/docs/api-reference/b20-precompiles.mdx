...
Every selector on this page is final, but not all of them are dialable today. [Beryl](/specifications/beryl-overview/) shipped the standard and is live on Mainnet and Sepolia. A second group arrives with [Cobalt](/specifications/cobalt-overview/), which has no activation timestamp yet — call any of it before then and there is nothing at the address to answer.
...
Nothing you call today stops working, so none of this is urgent. Each item is a path worth moving to before Cobalt is live.
...
- **They read different policies.** `seizeWithMemo` consults `SEIZE_HOLDER_POLICY`; `burnBlocked` consults `TRANSFER_SENDER_POLICY`. A token can define a seizable set that has nothing to do with its transfer-blocked set. Under both, "eligible" means *not* authorized by the relevant policy.
...
One existing behaviour shifts. `createPolicy` and `createPolicyWithAccounts` gain a revert path that was previously unreachable: passing `UNION` or `INTERSECT` to a simple constructor now reverts `IncompatiblePolicyType`, checked right after `ZeroAddress`.
