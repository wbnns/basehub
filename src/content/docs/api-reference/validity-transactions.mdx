...
<Aside type="caution">
This is still experimental, but it now has a date. `base_sendRawTransactionValidity` runs on Vibenet only today, and it reaches the public networks with [Cobalt](/specifications/cobalt-overview/): Base Sepolia on September 23, 2026 and Mainnet on September 30, 2026. Treat the shape of the API as unsettled until then. Upstream has not published the final specification, error codes and messages are not stable, and the production contract can still move.
</Aside>
