...
sources:
- base/docs:docs/base-chain/api-reference/rpc-overview.mdx
---
...
<Aside type="note">
The public endpoints are rate-limited and not suitable for production traffic. Connect through a node provider for production workloads.
</Aside>

<Aside type="caution">
The public Base endpoints are **HTTP only**. WebSocket RPC — `eth_subscribe` and the `newHeads`, `logs`, and Flashblocks streams — is not served on the public endpoints. For WebSocket support, use a node provider such as Alchemy or QuickNode.
</Aside>
...
| Method | Description |
| --- | --- |
| [optimism_outputAtBlock](/api-reference/rollup-node-rpc/) | Output root and sync metadata for an L2 block |
