...
<Aside type="tip">
**Flashblocks:** set `"fromBlock": "pending"` and `"toBlock": "pending"` against `https://mainnet.base.org` to query logs from pre-confirmed transactions, refreshed every ~200ms. For a real-time stream, consider the [`pendingLogs`](/api-reference/flashblocks/pendinglogs/) WebSocket subscription instead.
</Aside>
...
| --- | --- | --- |
| `fromBlock` | string | Start of the block range. Block number in hex or a block tag. Pass `"pending"` to include pre-confirmed logs. Defaults to `"latest"`. |
...
| `blockTimestamp` | string | Unix timestamp (hex) of the block that contains this log. An OP Stack extension to the standard Ethereum log schema. |
