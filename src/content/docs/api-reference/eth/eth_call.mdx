...
<Aside type="tip">
**Flashblocks:** call `https://mainnet.base.org` with `"pending"` to simulate against the current pre-confirmed block state, refreshed every ~200ms.
</Aside>
...
Block number in hex, or `"latest"`, `"pending"`, `"safe"`, `"finalized"`, `"earliest"`. Pass `"pending"` to call against pre-confirmed state.
