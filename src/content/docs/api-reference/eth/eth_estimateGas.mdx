...
sources:
- base/docs:docs/base-chain/api-reference/ethereum-json-rpc-api/eth_estimateGas.mdx
---
...
<Aside type="tip">
**Flashblocks:** call `https://mainnet.base.org` with `"pending"` to estimate gas against the current pre-confirmed state — useful when a new transaction depends on one that has just been pre-confirmed.
</Aside>
...
Block to estimate against. Defaults to `"latest"`. Pass `"pending"` to estimate against pre-confirmed state.
