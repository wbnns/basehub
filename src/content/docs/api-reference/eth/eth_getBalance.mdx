...
<Aside type="tip">
**Flashblocks:** call `https://mainnet.base.org` with `"pending"` to read balances refreshed every ~200ms — before the block seals.
</Aside>
...
| 2 | `block` | string | Block number in hex, or `"latest"`, `"pending"`, `"safe"`, `"finalized"`, `"earliest"`. Pass `"pending"` for the pre-confirmed balance. Required. |
