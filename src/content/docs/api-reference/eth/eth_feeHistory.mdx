...
| `baseFeePerGas` | array | Base fees per gas for each block, with one extra trailing entry for the next pending block, so the length is `blockCount + 1`. |
| `gasUsedRatio` | array | Gas-used to gas-limit ratios for each block (0.0 to 1.0); length is `blockCount`. |
| `baseFeePerBlobGas` | array | Base fees per blob gas for each block plus one extra entry for the next pending block (EIP-4844), giving a length of `blockCount + 1`. Currently always `"0x1"` on Base. |
| `blobGasUsedRatio` | array | Blob-gas-used ratios for each block (0.0 to 1.0), used to derive the next blob base fee (EIP-4844); length is `blockCount`. |
