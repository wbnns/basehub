...
sources:
- base/docs:docs/base-chain/api-reference/ethereum-json-rpc-api/eth_getTransactionByHash.mdx
---
...
| `maxFeePerGas` | string | EIP-1559 maximum total fee per gas (hex). Present for type `0x2` transactions only; absent on type `0x7e`. |
| `maxPriorityFeePerGas` | string | EIP-1559 maximum priority fee per gas (hex). Present for type `0x2` transactions only; absent on type `0x7e`. |
...
| `chainId` | string | Chain ID the transaction is valid for. `"0x2105"` for Base Mainnet, `"0x14a34"` for Base Sepolia. Not present on type `0x7e`. |
...
| `r` | string | 32-byte ECDSA signature component r (hex). Always `"0x0"` for type `0x7e`. |
| `s` | string | 32-byte ECDSA signature component s (hex). Always `"0x0"` for type `0x7e`. |
| `yParity` | string | Signature parity (hex). Always `"0x0"` for type `0x7e` deposits. |
| `sourceHash` | string | Identifies the deposit source (32-byte hex). Present on type `0x7e` only. |
| `mint` | string | ETH minted on L2 as part of the deposit (hex), usually `"0x0"`. Present on type `0x7e` only. |
| `depositReceiptVersion` | string | Version of the deposit receipt format (hex). Present on type `0x7e` only. |
