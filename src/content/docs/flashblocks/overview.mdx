...
<Aside type="caution" title="Denim will retire Flashblocks">
Upstream plans to deprecate Flashblocks in the [Denim](/specifications/denim-200ms-blocks/) hardfork, which turns each 200ms interval into a canonical block and ends Flashblocks production. Upstream briefly filed this retirement under Cobalt in early September 2026 and has since moved it back to Denim. Upstream lists October 2026 as a planning target. The upgrade is not finalised, and nothing is scheduled on Sepolia or Mainnet. Canonical 200ms blocks are live on Vibenet, so you can test the new behaviour early. [Migrate to 200ms Blocks](/flashblocks/migrate-to-200ms-blocks/) lists the application changes each integration eventually needs. Everything else on this page describes how Base works today.
</Aside>
...
For details on the sequencer system and the Flashblocks infrastructure components (rollup-boost, base-builder, websocket-proxy, and base), see the [Architecture](/flashblocks/architecture/) page.
...
The mempool keeps P2P connections open with execution layers (base-reth-node, base-builder), keeping all pending transactions in sync for block building.

### 3. Block building

In each 200ms loop, base-builder selects transactions using:
