...
<Aside type="caution" title="Cobalt will retire Flashblocks">
Upstream now plans to deprecate Flashblocks in the [Cobalt](/specifications/cobalt-200ms-blocks/) hardfork, which turns each 200ms interval into a canonical block and ends Flashblocks production. This retirement was attributed to Denim until early September 2026, when upstream reassigned 200ms blocks to Cobalt. The upgrade is not finalised, and nothing is scheduled on Sepolia or Mainnet. Canonical 200ms blocks are live on Vibenet, so you can test the new behaviour early. [Migrate to 200ms Blocks](/flashblocks/migrate-to-200ms-blocks/) lists the application changes each integration eventually needs. Everything else on this page describes how Base works today.
</Aside>
