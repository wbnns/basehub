...
Inclusion timing is driven mainly by priority fee, not transaction size. The builder allocates gas cumulatively — Flashblock `j` may use up to `j/10` of the total block gas limit — so in principle a very large transaction has a harder time landing in the first Flashblock. In practice this rarely bites: Base's [per-transaction gas maximum](/network/block-building/#per-transaction-gas-maximum) of 16,777,216 gas (~16.7M) is already below Flashblock 1's ~40M budget, so any valid transaction fits in the first Flashblock on size alone. When a transaction is slow to include, the usual cause is a low priority fee relative to others competing in the same 200ms window.

See [Block Building](/network/block-building/) for the gas budget breakdown and throughput limits.

### How do I make sure my transaction lands in the first Flashblock?

There is no way to guarantee inclusion in a specific Flashblock, just as there is no way to guarantee a specific block. Since the per-transaction gas maximum (~16.7M) sits below Flashblock 1's ~40M capacity, gas size is not the limiting factor — every valid transaction is eligible for the first Flashblock. To improve the odds of fast inclusion, set a higher priority fee.
...
Whatever socket you are connected to belongs to somebody, and it enforces their limits. Base publishes no free WebSocket RPC on Mainnet or Sepolia, so every `wss://` endpoint that works for Flashblocks is either your own node or a provider's, and each sets its own ceiling on connections and messages. There is no shared public allowance to be throttled against. For production use:

1. Run your own [Flashblocks-aware RPC node](/node-operations/run-a-base-node/#enable-flashblocks), where the limits are yours to set.
2. Use a third-party node provider with Flashblocks support, and size the plan to your subscription count.
...
Use the Reth binary from the [base/base repository](https://github.com/base/base). Full instructions are in [Enable Flashblocks](/node-operations/run-a-base-node/#enable-flashblocks).
