...
description: Onchain contracts that verify proof material, track game state, and release withdrawals for the Azul proof system.
sources:
- base/docs:docs/base-chain/specs/protocol/proofs/contracts.mdx
---
...
<Aside type="caution" title="Changed at Cobalt">
This section and the `NitroEnclaveVerifier` section below describe the pre-Cobalt, ZK-proof registration path. From Cobalt, the registry exposes `NITRO_VALIDATOR()` and registers signers with `registerSigner(bytes attestationTbs, bytes signature, bytes hints)` (selector `0xb39dc09d`). The attestation is checked in full onchain by `NitroValidator`, `CertManager`, and `P384Verifier`, and certificate revocation moves to `CertManager`. See the [TEE Prover Registrar](/specifications/registrar/) and the [nitro-validator redesign](/architecture/nitro-validator/).
</Aside>
