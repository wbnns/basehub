...
- **B20 policy and receiver changes** — the node source also gates two B20 changes on Denim, though upstream's fork page does not list them. The policy registry gains inverted policy IDs (bit 63 as a NOT flag, plus `invertedPolicyId`). Tokens also refuse to credit their own address. Both are in [Arriving with Denim](/api-reference/b20-precompiles/#arriving-with-denim). A third change, B20 logic version 3, enforces `TRANSFER_EXECUTOR_POLICY` on every transfer path, including a holder's own `transfer`; see [B20 Issuer Operations](/integration-guides/b20-issuer-operations/#limit-who-may-initiate-transfers).
