---
title: Cobalt 200ms Native Blocks
description: Specification for the Cobalt upgrade's canonical 200ms blocks, where every 200ms interval becomes a full Base block and BaseTime supplies the sub-second timestamp.
sources:
...
Cobalt changes how Base produces blocks. Instead of one canonical block every two seconds, the chain would seal five per second. Every 200ms interval would produce a block that is complete in its own right — separately numbered and hashed, with its own state root and receipts, its own forkchoice update, and its own path from unsafe to safe to finalized.
...
Cobalt is active on neither Sepolia nor Mainnet. No activation time is set for either, and the required client versions are still open.

Which fork ships this is genuinely unsettled, so treat the name as provisional. The published upgrade docs place 200ms blocks under Cobalt, having moved them out of **Denim** in early September 2026. The node implementation says otherwise: as of 2026-09-10 every block-timing path is gated on `is_denim_active`, and a commit that day restored that gate after it had been pointed at Cobalt. Neither fork has an activation timestamp on any built-in chain, so nothing here is live outside Vibenet regardless of the label. None of the mechanics below depend on the outcome — the BaseTime deposit, the 200ms cadence, and the Flashblocks retirement are the same either way.
</Aside>

## What Cobalt changes

Today [Flashblocks](/flashblocks/overview/) publish incremental previews of pending state inside a single two-second block. Cobalt removes the need for that layer by making the 200ms interval a real block. Under the rollout Base stops emitting Flashblocks entirely, so anything reading a Flashblocks stream has to move over to canonical block and RPC feeds.
...
Upstream lists **late October 2026** as the planning target for Cobalt on both Sepolia and Mainnet. That is a target under a planning status rather than a booked activation: no timestamp is published for either network, and the required client versions are still open. The node source agrees, leaving `cobalt_timestamp` unset on every canonical network. Treat the month as subject to change until timestamps appear.
...
On `forkchoiceUpdated` the execution client validates what it can without touching contract state: the millisecond part is absent before Cobalt, present and one of the five legal values after, and the whole-second timestamp is not behind the parent's.
...
After activation, derivation computes timestamps from the Cobalt schedule and the absolute L2 block number. It reads the millisecond part neither from batch data nor from the local clock, which is what keeps the result deterministic. Blocks walk the second in five steps:
...
<Aside>
The behavior in this section is planned for Cobalt. None of it is live on production endpoints.
</Aside>
...
Pruned or unprovenanced transaction and log data behaves the same way. Keeping all of these optional is what lets pre-Cobalt history, and clients lacking the body data, still be represented.

### Tooling

Foundry can carry an unknown block-level `timestampMs` through its `AnyRpcBlock` and `OtherFields` paths while EVM execution keeps working in seconds. Plain Alloy `AnyRpcHeader` discards unknown fields, so anything that needs Cobalt timestamps should reach for `WithOtherFields<AnyRpcHeader>` or a typed Base response instead. Anvil is not expected to attach BaseTime metadata to locally mined blocks in the first rollout.

## What this means for integrators

Applications reading Flashblocks preconfirmations are the ones with work to do, because that stream goes away rather than changing shape. The replacement is the canonical block feed, which after Cobalt arrives at the same 200ms cadence the Flashblocks stream did — with the difference that each update is a sealed block rather than a preview of a pending one.
