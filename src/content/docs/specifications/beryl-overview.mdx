...
Builds for both networks are now published — the Mainnet versions have been confirmed ahead of its activation date.
...
<Aside type="caution">
Operators need to act before activation. Make sure both your Sepolia and Mainnet nodes are on the required `base-reth-node` build ahead of each network's date in the table above.
</Aside>
...
Account abstraction moves into the protocol itself, but it is not part of Beryl. It was slated for [Cobalt](/specifications/cobalt-overview/), the hardfork that comes next, until the node source moved it onto a separate activation gate in September 2026 — one that no public network sets. Nothing has been scheduled in its place. Each account records — onchain — who is allowed to act for it and how its signatures are checked, so smart-account behavior no longer depends on bundlers, relays, or a separate mempool. That hands applications portable smart accounts out of the box, along with scoped, revocable session keys, atomic call batching, and gas sponsorship paid natively by the protocol. EIP-8130 is still experimental and, for now, runs only on the vibenet devnet. The full mechanics — the account/actor/policy/authenticator model and a working vibenet example — are in [Native Account Abstraction](/architecture/native-account-abstraction).
