...
<Aside type="caution">
A published date and an onchain activation timestamp are two different things, and right now Cobalt has the first but not the second. Do not read that gap as a slipped schedule. `cobalt_timestamp` is unset on every canonical network in the node source, and it stays unset, because Cobalt is one of the upgrades that carries its own schedule onchain. The client registers Cobalt as contract-backed under the fork alias `v3` and reads its activation from the L1 registry at runtime. So no client release will arrive carrying these dates compiled in, and waiting for one is the wrong signal to watch.
</Aside>
...
Upstream has not published a minimum client version for Cobalt yet, so there is still nothing specific to install. Watch for that announcement rather than for a release that hardcodes the fork times: because Cobalt activates from the onchain registry, a node can reach the date on a build that predates it. To exercise the new surface before either date, point a client at [vibenet](/introduction/connecting-to-base/).
