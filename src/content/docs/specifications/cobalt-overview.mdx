...
Upstream moved Cobalt off planning status on 2026-09-15 and named both dates. Both are now pinned in the node source rather than merely announced, and the status page has published a formal scheduled-event notice for Sepolia, so the scope on this page is no longer expected to move.
...
<Aside type="caution">
The two networks have different minimum releases, and the Mainnet one is not a published release yet. `v1.4.0` covers Sepolia. For Mainnet you need a build at or after `v1.4.1-rc.9` — see [Required software](#required-software) before September 30.
</Aside>
...
Mainnet now has a schedule in the source, but no stable release carries it. The commit that set chain `8453` to `1790791200` landed on 2026-09-21 at 15:24 UTC. `v1.4.0` was tagged five days earlier, so it ships Mainnet `cobalt_timestamp: None` — a Mainnet node on `v1.4.0` recognises no Cobalt activation and will not follow the fork on September 30.

The first tag containing the Mainnet timestamp is `v1.4.1-rc.9`, cut nine minutes after that commit; `v1.4.1-rc.8` and everything before it still read `None`. As of this writing there is no `v1.4.1` final release, only candidates. So the Mainnet floor is a real version boundary but not yet an installable stable build, and the gap has to close inside the nine days before activation. Watch the [releases page](https://github.com/base/base/releases) for `v1.4.1`.

<Aside type="caution">
Do not read "`v1.4.0` or later" as sufficient for both networks. It is the published floor for Sepolia only. The Mainnet schedule was added after that tag, so upgrading to `v1.4.0` satisfies September 23 and leaves September 30 unmet.
</Aside>

One packaging note worth repeating, because the September 23 window is the first fork where it bites: the public node images and binaries now come from `github.com/base/base`. The retired `base/node` repository stopped receiving them at `v1.3.0`, so there is no `v1.4.0` on the old path to pull. To exercise the new surface before either date, point a client at [vibenet](/introduction/connecting-to-base/).
