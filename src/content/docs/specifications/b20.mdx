...
| `SEIZE_HOLDER_POLICY` | The `from` of `seizeWithMemo`. Note the inversion — a holder is only seizable while this scope *denies* them |
...
- `from` must be **denied** by `SEIZE_HOLDER_POLICY`. Authorization here marks an account as protected, so only accounts the policy turns away can be seized from.
...
Pausing is fine-grained: the `PausableFeature` enum splits the token surface into operations that pause independently — `TRANSFER`, `MINT`, `BURN`, and `SEIZE`. That enum only ever grows. By design, `pause(features)` and `unpause(features)` answer to different roles (`PAUSE_ROLE` and `UNPAUSE_ROLE`).
