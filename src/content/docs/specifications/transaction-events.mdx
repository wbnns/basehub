...
Journaling is split across two devnet layers. The core devnet — `just devnet up` or `just devnet up-single` — switches on durable node journals for `base-client` and `base-builder`, which write their JSONL under `.devnet/transaction-events/`. The tx-observability overlay then layers the collection side on top: a Vector shipper, a local Postgres, the `audit-archiver` ingest path, and the proxyd producer. That overlay never touches the node journal configuration — it only consumes what the core layer already emits.
...
The smoke test pushes a single transaction through proxyd, waits for Vector to deliver the JSONL records from the proxyd, txpool-tracing, and builder producers, and then confirms `audit-archiver` can read the stored events back out of Postgres by transaction hash.

:::caution
This overlay was called `ingress` until 2026-09-21, when the `ingress-rpc` service was deleted from `base/base`. The recipe was renamed rather than aliased, so `just devnet ingress` no longer resolves — along with `tx-observability`, the working recipes are now `tx-observability-down` and `tx-observability-logs`. The compose file moved to `etc/docker/docker-compose.tx-observability.yml`.
:::
...
- `base-builder`
- `base-routing/proxyd`
- `ingress-rpc` (retired; the value stays defined so archived events remain readable)

Nothing emits `ingress-rpc` any more. The service was removed on 2026-09-21, but its producer value and event types were deliberately left in the vocabulary, because `audit-archiver` rows written before that date still carry them and would otherwise fail to parse. Treat it as a value you may read and must never write.
...
Ingress/audit (retired producer; the types are retained so historical events remain readable):
...
## Mempool and Builder Examples

These three events trace one validity transaction from admission through to a builder-side discard.
...
A recoverable predicate parks the transaction rather than dropping it, holding it for a later ordering position or a later flashblock. Note what is absent — the predicate list is not restated here:
...
Expiry is the builder's terminal discard, written once no later attempt can satisfy the predicate. Exhausting parking capacity is a different outcome and still surfaces as `BUILDER_REJECTED` with `validity_predicate_not_satisfied`:
