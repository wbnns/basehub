...
For Base Mainnet (chain ID 8453), these resolve to `/optimism/8453/0/blocks` through `/optimism/8453/3/blocks`. The version is selected based on the active hardfork (a protocol upgrade that changes network rules, activated at a specific timestamp) at the block's timestamp. V1 covers pre-Canyon blocks, V2 Canyon/Delta, V3 Ecotone, and V4 Isthmus. Each version uses a slightly different encoding for the execution payload envelope.

A node does not hold all four subscriptions forever. Topics retire as forks age out. `BlockHandler::MAX_BLOCK_AGE` is 60 seconds, and it does double duty: it caps how old a gossip block can be, and it sets the retirement grace period. Once a replacement fork has been active for longer than that window, no block on the older topic can still be inside the accepted age range, so the subscription serves no purpose and the driver drops it:

```rust
pub const MAX_BLOCK_AGE: u64 = 60;
```

`topics_at()` computes the live set by finding the topic that would carry a block from 60 seconds ago and discarding everything older. Two properties of that calculation are worth holding onto. It reads the **local** clock, never a timestamp a peer claims, so a peer cannot talk a node into reviving a dead topic. And it keeps future topics joined ahead of their activation, so the mesh for the next fork is already warm when the fork lands. `GossipDriver` re-evaluates the set every 5 seconds (`TOPIC_RETIREMENT_INTERVAL`) whether or not traffic is arriving, and skips topics that are already retired when it starts up.

Retirement binds in both directions, and cheaply. Inbound, the handler discards a message on a retired topic before it decompresses or SSZ-decodes anything, so a flood of obsolete-topic traffic costs almost nothing to refuse. Outbound, a retired topic cannot be encoded for publication — relevant because gossipsub will happily publish to a topic you are not subscribed to, and this closes that path. Only v1 through v3 are ever retired; v4 is the newest topic and always stays.

<Aside type="caution">
Retirement is one-way within a process. If the local clock jumps backwards, topics already dropped are **not** rejoined. A node whose time source corrects itself downward past a fork boundary will therefore sit without the subscription it now needs, and nothing in the node reports this. Restart it after any large backwards clock correction.
</Aside>

With the `metrics` feature on, the live set is observable. `base_node_block_topic_subscribed`, `base_node_block_topic_peers`, and `base_node_block_topic_mesh_peers` are each labelled by version, and `base_node_block_topic_blocked_total` counts refusals with an `inbound` or `outbound` direction label. The three gauges are sampled at startup and on every retirement check, including while the network is idle, and they publish explicit zeroes for retired topics rather than dropping the series. Watch `peers` against `subscribed`: `peers` counts neighbours still advertising a topic even after the local node has retired it, so the two diverging is the expected shape during a fork transition, not a fault.
...
Field order carries meaning here. libp2p calls each behaviour's connection hooks in declaration order, so `connection_limits` sits first: it turns away surplus connections before any other behaviour allocates a handler for them. Its caps default to 5 pending inbound, 16 pending outbound, 30 established in each direction and in total, and 1 established connection per peer. This is a different layer from the `ConnectionGater` above. The gater decides whether a given peer is allowed to connect at all. These limits bound how many connections exist, whoever they belong to.

`ping` sends periodic keepalive pings and measures round-trip times. `gossipsub` handles block gossip. `identify` exchanges capability information when peers first connect (Base advertises its agent version as `"base"`).

Notice what is absent. Base neither advertises nor implements `payload_by_number`, the legacy op-node request-response protocol. Earlier releases carried a `sync_req_resp` stub that answered every such request with "not found", purely so op-nodes would not penalize Base for the gap. That stub is gone, and a test now asserts no advertised protocol begins with `/opstack/req/payload_by_number/`. Nothing about block delivery regresses as a result: gossipsub moves consensus-layer blocks, the execution layer syncs over its own path, and follow mode fetches payloads over HTTP. Any tooling that reached for this protocol on a Base node was already getting "not found" and must use one of those three paths instead.
...
The [`BaseNetworkBuilder`](https://github.com/base/base/blob/main/crates/execution/node/src/node.rs) configures Reth's network for Base. Since transaction gossip became unconditional it carries a single knob:
...
Transaction pool gossip is no longer something a node decides for itself. Base sets `tx_gossip_disabled` unconditionally, so a node never broadcasts its pending transactions to DevP2P peers. The reasoning is structural rather than situational: in a rollup only the sequencer orders transactions, so relaying pending ones around the mesh achieves nothing that forwarding to the sequencer does not already do.
...
A node that is not the sequencer cannot include transactions itself, so whatever lands in its mempool has to be relayed onward to the sequencer. The reader/forwarder pipeline that handles this now lives in a dedicated `tx-forwarding` crate, split out of `txpool`.

The [`DestinationReader`](https://github.com/base/base/blob/main/crates/execution/tx-forwarding/src/reader/task.rs) polls the transaction pool for pending transactions and queues them onto a bounded `tokio::mpsc` channel. The [`DestinationForwarder`](https://github.com/base/base/blob/main/crates/execution/tx-forwarding/src/forwarder/task.rs) drains that queue and relays each transaction to the configured builder endpoints through a custom JSON-RPC method (`base_insertValidatedTransaction`). A separate forwarder task is spawned for each builder URL, so several downstream builders can be fed at once. Both halves of the pipeline run in the background, scheduled on the node's task executor.
...
Three rollup flags remain. `--rollup.sequencer` names the endpoint that forwarded transactions are sent to. `--rollup.discovery.v4` brings back discv4, which stays off unless you ask for it because discv5 is what Base runs. `--rollup.txpool-ordering` picks the pool's ordering strategy, either `coinbase-tip` or `timestamp`. Nothing here governs gossip any more — the `--rollup.disable-tx-pool-gossip` flag that used to pair with `--rollup.sequencer` has been withdrawn, along with the builder field behind it.
...
Submission runs the other way. A transaction arrives at some EL node's RPC endpoint and stops there as far as the mesh is concerned, because pool gossip is off everywhere. Where the node knows a sequencer endpoint, the forwarder hands the transaction straight over by JSON-RPC. From that point it rejoins the path already described: the sequencer seals it into a block, and the block travels back out over CL gossip.
...
- [`crates/execution/tx-forwarding/src/reader/`](https://github.com/base/base/tree/main/crates/execution/tx-forwarding/src/reader) — Transaction pool reader
- [`crates/execution/tx-forwarding/src/forwarder/`](https://github.com/base/base/tree/main/crates/execution/tx-forwarding/src/forwarder) — Transaction forwarder to sequencer
