...
This page traces the life of a transaction through the Base node, from the moment it arrives at the RPC endpoint to when it is included in a sealed block and committed to the state trie. Each stage references the crate responsible for that step.
