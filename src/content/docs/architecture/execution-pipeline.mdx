...
- In production deployments a proxy sits in front of the node, handling rate limiting, authentication (via `base-jwt`), and request routing before forwarding validated requests to the node's internal RPC server. The crate that did this, [`ingress-rpc-lib`](https://github.com/base/base/tree/v1.4.0/crates/infra/ingress-rpc), was removed on 2026-09-21 and the link points at `v1.4.0`, the last release containing it. The stage itself is unchanged; only the component that fills it moved out of this repository.
