...
After Fusaka, Base verified Nitro attestations through zero-knowledge proofs, the only practical route at the time. The hinted P-384 approach removes that dependency: no new precompile, no ZK proving service. That plan has now shipped. The [Cobalt](/specifications/cobalt-overview/) upgrade moves TEE signer registration for the proof system onto these hinted contracts, and the [registrar](/specifications/registrar/) now computes the hints itself instead of ordering a ZK proof.
